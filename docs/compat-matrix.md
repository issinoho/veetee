...
| Stored only | 🟡 | Receive speed and XOFF threshold (the transmit speed, data format, stop bits and flow control set the serial line, docs/serial-setup.md), modem control and speeds, transmit rate limits, refresh rate, printer assignment, user features lock, zero style, energy saver, host wake-up, overscan, NUL and half duplex, compose/Alt/F5/comma/angle/tilde key options and auto repeat rate are kept, saved, restored and reported (DECRQSS, DECRQM) but do not change behaviour yet. Set-Up is shown in English only |
